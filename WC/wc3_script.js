...
window.addEventListener("load",function() {
	...
	$('#slider').roundSlider({
		...
		change: function() {
			state.wc3.light_value = $("#slider").roundSlider("option", "value");
			stateJSON = JSON.stringify(state);
			localStorage.setItem("state", stateJSON);
			b = parseInt((-(2.55*state.wc3.light_value)+255)).toString(16)
			if (b.length < 2) {
	...
	$('#demo').html(state.wc3.water_value);
	$('#myRange').val(state.wc3.water_value);
	$('#myRange').on('input', function() {
		state.wc3.water_value = parseInt(this.value);
		$('#demo').html(state.wc3.water_value);
		stateJSON = JSON.stringify(state);
		localStorage.setItem("state", stateJSON);
	});
	...
	function addProgramLight(e) {
		...
		if (year_start == 2018) {
			if (month_start == 12) {
				i = day_start - 11;
				calendar[i].push([datestart, "WC3", "Lights", $('#light_value').val()+"%"]);
	...
	function addProgramWater(e) {
		...
		var [hours_start, minutes_start] = $('#time_start').val().split(":");
		var datestart = new Date(year_start, (month_start-1), day_start, hours_start, minutes_start, 0, 0);
		program.wc3.water.push(datestart);
		program.wc3.water_value.push($('#water_value').val());
		programJSON = JSON.stringify(program);
		localStorage.setItem("program", programJSON);
		if (year_start == 2018) {
			if (month_start == 12) {
				i = day_start - 11;
				calendar[i].push([datestart, "WC3", "Water", $('#water_value').val()+"ºC"]);
				calendarJSON = JSON.stringify(calendar);
				localStorage.setItem("calendar", calendarJSON);
			}
		}
