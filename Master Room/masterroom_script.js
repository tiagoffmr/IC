...
window.addEventListener("load",function() {
	...
	} else if (state.mroom.light === false) {
	...
	$('#slider').roundSlider({
		...
		change: function() {
			state.mroom.light_value = $("#slider").roundSlider("option", "value");
			stateJSON = JSON.stringify(state);
			localStorage.setItem("state", stateJSON);
			b = parseInt((-(2.55*state.mroom.light_value)+255)).toString(16)
			if (b.length < 2) {
	...
	blinds(state.mroom.blinds);
	
	if (state.mroom.blinds == 0){
		$('#topButton').toggle();
		$('#upButton').toggle();
	}
	if (state.mroom.blinds == 4){
		$('#downButton').toggle();
		$('#bottomButton').toggle();
	}
	...
	$('#upButton').click(function(event) {
		...
		state.mroom.blinds = parseInt(state.mroom.blinds) - 1;
		if (state.mroom.blinds == 0){
			$('#topButton').toggle();
			$('#upButton').toggle();
		}
	...
	function addProgramLight(e) {
		...
		if (year_start == 2018) {
			if (month_start == 12) {
				i = day_start - 11;
				calendar[i].push([datestart, "Master Room", "Lights", $('#light_value').val()+"%"]);
	...
	function addProgramBlinds(e) {
		...
		if (year_start == 2018) {
			if (month_start == 12) {
				i = day_start - 11;
				calendar[i].push([datestart, "Master Room", "Blinds", s.options[s.selectedIndex].value]);
				calendarJSON = JSON.stringify(calendar);
				localStorage.setItem("calendar", calendarJSON);
			}
		}
