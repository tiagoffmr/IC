...
window.addEventListener("load",function() {
	...
	} else if (state.room1.light === false) {
	...
	$('#slider').roundSlider({
		...
		change: function() {
			state.room1.light_value = $("#slider").roundSlider("option", "value");
			stateJSON = JSON.stringify(state);
			localStorage.setItem("state", stateJSON);
			b = parseInt((-(2.55*state.room1.light_value)+255)).toString(16)
			if (b.length < 2) {
	...
	blinds(state.room1.blinds);
	
	if (state.room1.blinds == 0){
		$('#topButton').toggle();
		$('#upButton').toggle();
	}
	if (state.room1.blinds == 4){
		$('#downButton').toggle();
		$('#bottomButton').toggle();
	}
	...
	$('#upButton').click(function(event) {
		...
		state.room1.blinds = parseInt(state.room1.blinds) - 1;
		if (state.room1.blinds == 0){
			$('#topButton').toggle();
			$('#upButton').toggle();
		}
	...
	function addProgramLight(e) {
		...
		if (year_start == 2018) {
			if (month_start == 12) {
				i = day_start - 11;
				calendar[i].push([datestart, "Room 1", "Lights", $('#light_value').val()+"%"]);
	...
	function addProgramBlinds(e) {
		...
		if (year_start == 2018) {
			if (month_start == 12) {
				i = day_start - 11;
				calendar[i].push([datestart, "Room 1", "Blinds", s.options[s.selectedIndex].value]);
				calendarJSON = JSON.stringify(calendar);
				localStorage.setItem("calendar", calendarJSON);
			}
		}
